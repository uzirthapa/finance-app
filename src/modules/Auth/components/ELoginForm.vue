    ...
    <v-card-text>
      <v-form ref="form" v-model="valid" lazy-validation>
        ...
        <span class="caption grey--text text--darken-1">
          This is the email you will use to login to your DTM
          account
        </span>
        <v-text-field
          v-model="password"
      ...
      <div class="mb-3">Or Sign in with:</div>
      <v-btn color="red darken-2" dark class="text-none mr-2" @click="socialLogin('google')">
        <v-icon left>fab fa-google</v-icon>Google
      </v-btn>
      <v-btn color="black" dark class="text-none mr-2" @click="socialLogin('apple')">
        <v-icon left>fab fa-apple</v-icon>Apple
      </v-btn>
      <!-- <v-btn
        color="blue darken-2"
        dark
    ...
    <v-card-actions>
      <v-dialog v-model="reset" width="500">
        <template v-slot:activator="{ on }">
          <v-btn text class="text-none font-weight-regular" @click="reset = true">Forgot my password</v-btn>
        </template>
